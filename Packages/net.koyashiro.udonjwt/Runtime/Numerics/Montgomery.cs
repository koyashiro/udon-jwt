...
namespace Koyashiro.UdonJwt.Numerics
{
    public class Montgomery : UdonSharpBehaviour
    {
        ...
        {
            var e = exponent;
            var @base = MontgomeryReduction(UnsignedBigInteger.Multiply(value, _r2));
            var buf = MontgomeryReduction(_r2);

            while (e > 0)
            {
                if (e % 2 != 0)
                {
                    buf = MontgomeryReduction(UnsignedBigInteger.Multiply(buf, @base));
                }
                @base = MontgomeryReduction(UnsignedBigInteger.Multiply(@base, @base));
                e >>= 1;
            }

            return MontgomeryReduction(buf);
        }

        public uint[] MontgomeryReduction(uint[] t)
        {
